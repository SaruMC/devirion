 ...
 *
 * @author poggit
 * @link https://github.com/saru-mc/devirion
 *
 *
 */
...
function main() : void{
	...
	if(isset($opts["git"])){
		...
		$gitHash = $exitCode === 0 ? trim($output[0]) : 'unknown';
	...
	if(isset($opts["out"])){
		...
		$pharPath = getcwd() . DIRECTORY_SEPARATOR . "DEVirion.phar";
	}

	$stubContent = file_get_contents(__DIR__ . DIRECTORY_SEPARATOR . 'make-phar-stub.php');
	if($stubContent === false){
		echo "Failed to read stub file" . PHP_EOL;
		exit(1);
	}
